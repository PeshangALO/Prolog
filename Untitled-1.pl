...
outputFile('/Users/peshangalo/Desktop/puzzle_solved.txt ').
inputFile('/Users/peshangalo/Desktop/puzzle_unsolved.txt').
/********************* solving the puzzle */
doSolve(P,P).
:- use_module(library(clpfd)).
...
solvePuzzles(0).
solvePuzzles(N):- N>0, readProblem(P,X),
 length(X, SizeY),
  nth0(0, X, X1), length(X1, SizeX), NewSizeX is SizeX -1, NewSizeY is SizeY -1,
write(NewSizeY), write(', '), write(NewSizeX), nl,   maplist(label,X), maplist(portray_clause, X),nl,
